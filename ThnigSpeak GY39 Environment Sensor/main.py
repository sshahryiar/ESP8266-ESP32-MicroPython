...
from Thingspeak_GY39 import Thingspeak_GY39
...
display.print_str(6, 10, 2, display.WHITE, display.BLACK, "ESP GY39 ThingSpeak")

thingspeak = Thingspeak_GY39(Thingspeak_Credentials.API_write_key, Thingspeak_Credentials.API_read_key)
...
while(True):
    ...
    string_1 = "Lm/Lux: " + str("%3.2f" %l)
    string_2 = "Tmp/'C: " + str("%3.2f" %t)
    string_3 = "P/mbar: " + str("%3.2f" %p)
    string_4 = "R.H./%: " + str("%3.2f" %rh)
    string_5 = "Alt./m: " + str("%3.2f" %alt)
    ...
    display.print_str(6, 190, 2, display.WHITE, display.BLACK, "Last Update/s: " + str(3 * network_check_counter) + " ")
    ...
    if(network_check_counter < 0):
        ...
        if(wifi_station.isconnected() == True):
            thingspeak.push_data(l, t, p, rh, alt)
